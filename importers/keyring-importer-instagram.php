...
function Keyring_Instagram_User_Importer() {

	class Keyring_Instagram_User_Importer extends Keyring_User_Importer_Base {
		...
		function __construct() {
			parent::__construct();
			add_action( 'keyring_user_importer_instagram_custom_options', array( $this, 'custom_options' ) );
		}
		...
		function custom_options() {
			...
			<tr valign="top">
				<th scope="row">
					<label for="hashtags"><?php _e( 'Only import posts with the following hashtags (space separated)', 'keyring' ); ?></label>
				</th>
				<td>
					<input class="widefat" type="text" name="hashtags" id="hashtags" value="<?php echo esc_attr( $this->get_option( 'hashtags' ) ); ?>" />
				</td>
			</tr><?php
		}
		...
		function extract_posts_from_data( $raw ) {
			...
			foreach ( $importdata->data as $post ) {
				...
				$post_content = '<div class="instagram-image">';
				...
				// Check for desired hashtags
				$hashtags = array_map( function ( $tag ) {
					return trim( $tag, '\s #' );
				}, explode( ' ', $this->get_option( 'hashtags', '' ) ) );
				if ( $hashtags && ! array_intersect( array_map( 'strtolower', $hashtags ), array_map( 'strtolower', $tags ) ) ) {
					$this->set_option( 'total', intval( $this->get_option( 'total' ) ) - 1 );
					continue;
				}
		...
		function insert_posts() {
			...
			foreach ( $this->posts as $post ) {
				...
				if (
					...
					$this->sideload_media( $instagram_img, $post_id, $post, apply_filters( 'Keyring_Instagram_User_Importer_image_embed_size', 'full' ) );

					$imported++;

					do_action( 'keyring_user_post_imported', $post_id, static::SLUG, $post );
				}
			}
			$this->posts = array();
...
} // end function Keyring_Instagram_User_Importer


add_action( 'init', function () {
	Keyring_Instagram_User_Importer(); // Load the class code from above
	keyring_register_user_importer(
		'instagram',
		'Keyring_Instagram_User_Importer',
