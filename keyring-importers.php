<?php
/*
Plugin Name: Keyring Social User Importers
Description: Take back your content from different social media websites like Twitter, Flickr, Instagram, Delicious and Foursquare on a per user basis. Store everything in your own WordPress so that you can use it however you like.
Plugin URL: http://dentedreality.com.au/projects/wp-keyring-importers/
Version: 1.0
Author: Beau Lebens, Robert O'Rourke
...
abstract class Keyring_User_Importer_Base {
	...
	function __construct() {
		...
		if (
			( isset( $_REQUEST['page'] ) && $this->get_page_slug() == $_REQUEST['page'] )
			&&
			(
			...
			$this->reset();
			Keyring_Util::connect_to( static::SLUG, 'keyring-' . static::SLUG . '-user-importer-' . get_current_user_id() );
			exit;
		}
	...
	function set_option( $name, $val = null ) {
		if ( is_array( $name ) ) {
			...
			if ( is_null( $name ) && is_null( $val ) ) // $name = null to reset all options
			{
				$this->options = array();
			} else {
				if ( is_null( $val ) && isset( $this->options[ $name ] ) ) {
	...
	function handle_request() {
		...
		if ( empty( $_GET['page'] ) || $this->get_page_slug() != $_GET['page'] ) {
			return;
		}
	...
	function get_page_slug() {
		return 'keyring-user-' . static::SLUG;
	}
	...
	function get_form_action() {
		return 'admin.php?page=' . $this->get_page_slug();
	}
	...
	function done() {
		...
		$this->footer();
		do_action( 'user_import_done', 'keyring_' . static::SLUG );
		do_action( 'keyring_user_import_done', 'keyring_' . static::SLUG );
 ...
 */
function keyring_register_user_importer( $slug, $class, $plugin, $info = false ) {
	...
	$options = get_user_meta( get_current_user_id(), 'keyring_' . $slug . '_user_importer', true );
...
$keyring_user_importers = apply_filters( 'keyring_user_importers', $keyring_user_importers );
foreach ( $keyring_user_importers as $keyring_importer ) {
	require $keyring_importer;
}
unset( $keyring_user_importers, $keyring_importer );
